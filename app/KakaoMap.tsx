...
export default function KakaoPlaceSearch({ onSelect, onClose }: Props) {
    ...
    setTimeout(() => {
      ...
      if (userLocation) {
        ...
        myLocationMarkerRef.current = new window.kakao.maps.CustomOverlay({
          position: myPos, content: myMarkerContent, yAnchor: 0.5, xAnchor: 0.5,
        })
  ...
  function handleSearch(pageNum = 1) {
    ...
    if (!loaded) { setError('카카오맵 로딩 중이에요. 잠시 후 다시 시도해주세요.'); return }
    ...
    try {
      ...
      ps.keywordSearch(query, (data: Place[], status: string, pagination: any) => {
        if (status === window.kakao.maps.services.Status.OK) {
          setResults(data); setPage(pageNum)
          setHasNext(pagination.hasNextPage)
          setHasPrev(pagination.hasPrevPage)
          setError('')
        } else {
          setResults([]); setError('검색 결과가 없어요')
        }
      }, options)
    } catch (e) { setError('검색 중 오류가 발생했어요') }
  }
        ...
        {selectedPlace && (
          <div style={{ flexShrink: 0 }}>
            ...
            <div style={{ position: 'relative' }}>
              <div ref={mapContainerRef} style={{ width: '100%', height: '140px' }} />
              {/* 범례 */}
              <div style={{ position: 'absolute', top: '8px', left: '8px', backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: '8px', padding: '4px 8px', display: 'flex', flexDirection: 'column', gap: '3px', zIndex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '10px', color: '#444' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#4A90E2', border: '2px solid #fff' }} />내 위치
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '10px', color: '#444' }}>
                  <div style={{ width: '8px', height: '12px', backgroundColor: '#FF6B6B', borderRadius: '2px' }} />선택 업체
                </div>
              </div>
            </div>

            {/* 선택된 업체 정보 — 지도 밖으로 분리 */}
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFF5F5' }}>
              <div style={{ flex: 1, marginRight: '12px' }}>
                <p style={{ fontSize: '13px', fontWeight: '700', color: '#222', margin: '0 0 2px' }}>{selectedPlace.place_name}</p>
              ...
              <button
                ...
                style={{ padding: '8px 16px', borderRadius: '10px', backgroundColor: '#FF6B6B', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600', flexShrink: 0 }}
              >이 업체 선택</button>
            </div>
          </div>
        )}
        ...
        <div style={{ padding: '0 20px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px', paddingBottom: '12px' }}>
            {results.length === 0 && !error ? (
              <p style={{ color: '#bbb', textAlign: 'center', marginTop: '40px', fontSize: '14px' }}>업체명으로 검색해보세요 🔍</p>
            ) : (
              results.map((place, i) => (
                <div
                  key={i}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    {place.distance && (
                      <span style={{ fontSize: '12px', color: '#FF6B6B', fontWeight: '600', flexShrink: 0, marginLeft: '8px' }}>{formatDistance(place.distance)}</span>
                    )}
                  </div>
          ...
          {results.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px', paddingTop: '12px', borderTop: '1px solid #f0f0f0', marginBottom: '12px' }}>
              <button onClick={() => handleSearch(page - 1)} disabled={!hasPrev} style={{ padding: '6px 16px', borderRadius: '8px', border: '1px solid #f0f0f0', backgroundColor: !hasPrev ? '#fafafa' : '#fff', color: !hasPrev ? '#ccc' : '#444', cursor: !hasPrev ? 'default' : 'pointer', fontSize: '13px' }}>← 이전</button>
              <span style={{ fontSize: '13px', color: '#888' }}>{page} 페이지</span>
              <button onClick={() => handleSearch(page + 1)} disabled={!hasNext} style={{ padding: '6px 16px', borderRadius: '8px', border: '1px solid #f0f0f0', backgroundColor: !hasNext ? '#fafafa' : '#fff', color: !hasNext ? '#ccc' : '#444', cursor: !hasNext ? 'default' : 'pointer', fontSize: '13px' }}>다음 →</button>
            </div>
          )}
        </div>
      </div>
